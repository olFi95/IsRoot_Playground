...
use monte_carlo_root::nbody::nbody::{generate_test_data, NBody};
...
fn nbody_bench(c: &mut Criterion) {
    ...
    for steps in [1, 10, 100]{
        for initial_positions in [generate_test_data(100), generate_test_data(1000), generate_test_data(10000)]{
            ...
            c.bench_function(format!("NBodyGPU {} points {steps} step", initial_positions.len()).as_str(), |b| {
                b.to_async(FuturesExecutor).iter(|| async {
                    let mut nbody = NBodyGPU::new(&initial_positions).await;
                    nbody.step(steps).await;
                })
            });
            c.bench_function(format!("NBodyCPU {} points {steps} step", initial_positions.len()).as_str(), |b| {
                b.to_async(FuturesExecutor).iter(|| async {
